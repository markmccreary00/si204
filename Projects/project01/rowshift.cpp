...
int main(){
    ...
    //Check if fg would exceed bg height limits
    string header;
    ...
    if((fg_height + rowshift) > bg_height){
    ...
    for(int j=0;j<fg_height;j++){
        ...
        for(int i=0;i<(bg_width-fg_width);i++){
            int r_bg, g_bg, b_bg;
            fin2 >> r_bg >> g_bg >> b_bg;
            fout << r_bg << " " << g_bg << " " << b_bg << " ";
        }
